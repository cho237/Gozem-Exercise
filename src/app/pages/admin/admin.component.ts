...
export class AdminComponent implements OnInit, OnDestroy {
  ...
  onSavePackage(){
    ...
    if(this.editMode ){
      sub = this.packageService.updatePackage(this.package,this.package.package_id)
    }
    this.subs.add(
     sub.subscribe({
        next: (item)=> {
          const existingIndex = this.packages.findIndex((p) => p.package_id === this.package.package_id);
          if (existingIndex !== -1) {
  ...
  onDeletePackage(id:string){
    this.subs.add(
      this.packageService.deletePackage(id).subscribe({
        next: (res)=> {
          const updatedPackages = this.packages.filter((p) => p.package_id !== id);
  ...
  onDeleteDelivery(id:string){
    this.subs.add(
      this.deliveryService.deleteDelivery(id).subscribe({
        next: (res)=> {
          console.log(res);
          const updatedDeliveries = this.deliveries.filter((d) => d.delivery_id !== id);
  ...
  onSaveDelivery(){
    ...
    this.subs.add(
      this.deliveryService.createDelivery(this.delivery, this.delivery.package_id).subscribe({
      next: (delivery) => {
